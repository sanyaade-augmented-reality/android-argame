...
public class StartGame extends BaseActivity {
	...
	/**
	 * Simply route to the first sub-activity. This is defined here only so that
	 * we can have a consistent entry-point to the start-game set of activities.
	 */
	@Override
	...
	public static class BaseStartGameActivity extends GameMapActivity implements
			LocationListener {
		...
		public void onCreate(Bundle state) {
			...
			mapView.getOverlays().add(
					new DestinationOverlay(destinationReference,
							this.destinationDrawable));
		}
		...
		public void startActivity(Intent intent) {
			...
			if (destination != null) {
				Log.i("ZombieRun.BaseStartGameActivity",
						"Putting Destination to intent extras.");
	...
	public static class WaitingForFirstFixActivity extends
			BaseStartGameActivity {
		...
		public void onLocationChanged(Location location) {
			...
			if (location != null) {
				mapView.getController().animateTo(
						GeoPointUtil.fromLocation(location));
				mapView.getController().setZoom(15);
				startActivity(new Intent(this,
						ShowingPickDestinationMessageActivity.class));
			}
		}
	}

	public static class ShowingPickDestinationMessageActivity extends
			BaseStartGameActivity {
		@Override
		public void onCreate(Bundle state) {
			super.onCreate(state);

			FloatingPointGeoPoint lastKnownLocation = service
					.getHardwareManager().getLastKnownLocation();
			if (lastKnownLocation != null) {
				mapView.getController().animateTo(
						lastKnownLocation.getGeoPoint());
				mapView.getController().setZoom(15);
			}

			final Intent beginPickingDestinationIntent = new Intent(this,
					PickingDestinationActivity.class);
			new AlertDialog.Builder(this)
	...
	public static class PickingDestinationActivity extends
			BaseStartGameActivity {
			...
			new AlertDialog.Builder(this)
					...
					.setNegativeButton(getString(R.string.no),
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int which) {
									// Do nothing, just let another destination
									// be chosen.
								}
							}).show();
		}
		...
		private static class DestinationPickingOverlay extends Overlay {
			...
			public boolean onTap(GeoPoint point, MapView map) {
				...
				activity.destinationChosen(new Destination(
						new FloatingPointGeoPoint(point)));
				return true;
			}
		}
	}
